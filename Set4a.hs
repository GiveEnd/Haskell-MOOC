...
longest :: (Ord a) => [[a]] -> [a]
longest xss = foldr1 findLongest xss
  where
    findLongest xs ys
...
freqs = foldr (\x acc -> Map.alter (Just . maybe 1 (+1)) x acc) Map.empty
...
transfer from to amount bank
  ...
  | amount < 0 = bank
  | amount > balance = bank
  | otherwise = Map.adjust (\x -> x - amount) from $ Map.adjust (+amount) to bank
  where
    balance = Map.findWithDefault 0 from bank
